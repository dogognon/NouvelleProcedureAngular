...
import { DeleteDepartementComponent } from '../../param-base/departement/delete-departement/delete-departement.component';
import { EditDepartementComponent } from '../../param-base/departement/edit-departement/edit-departement.component';
import { NewDepartementComponent } from '../../param-base/departement/new-departement/new-departement.component';
...
export class EditProcedureComponent {
  ...
  public procedureEdit : procedure,public dialog: MatDialog,public dialogRef: MatDialogRef<NewProcedureComponent>,database: DynamicDatabase,private procedureService : ProcedureService,private dep : DepartementService,private cdRef: ChangeDetectorRef) {
    this.database = database;
    this.treeControl = new FlatTreeControl<DynamicFlatNode>(this.getLevel, this.isExpandable);
    this.dataSource = new DynamicDataSource(this.treeControl, database);
    this.dataSource.data = database.initialData();
    //initialiser les champs dans le formulaire
    this.nom=this.procedureEdit.binaire_id.fileName;
    this.commentaire=this.procedureEdit.commentaire;
   this.procedureService.downloadFile(this.procedureEdit.binaire_id.fileDownloadUri).subscribe({
      next : (data)=>{
        console.log(data)
        this.file = new File([data], this.procedureEdit.binaire_id.fileName, {type: "application/pdf"});
      },
      error : (err)=>{
        if(err.status===401){
